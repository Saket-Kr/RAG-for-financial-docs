...
class TableChunker:
    ...
    def _enrich_metadata(
        ...
        metadata = {
            ...
            "num_columns": len(table.headers) if table.headers else (len(table.data[0]) if table.data else 0),
        ...
        if context_before:
            metadata["context_before"] = context_before[-100:] if len(context_before) > 100 else context_before
        
        if context_after:
            metadata["context_after"] = context_after[:100] if len(context_after) > 100 else context_after
