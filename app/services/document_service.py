...
class DocumentService:
    def __init__(self, settings: Settings, metadata_service: MetadataService, storage: LocalStorage):
        ...
        self.relevance_checker = RelevanceChecker(
            settings.gatekeeping.threshold,
            settings.gatekeeping.rejection_message
        ) if settings.gatekeeping.enabled else None
        
        chunker_embedder = self.embedder if settings.chunking.strategy == "semantic" else None
    ...
    async def process_document(self, file_content: bytes, filename: str) -> str:
        try:
            ...
            try:
                ...
                self.metadata_service.update_document_metadata(document.id, {
                    **parsed_doc.metadata,
    ...
    async def query_document(self, document_id: str, query: str) -> Dict[str, Any]:
        try:
            ...
            if document.status != "completed":
                raise DocumentProcessingError(f"Document {document_id} is not ready for querying. Status: {document.status}")
            ...
            if self.relevance_checker:
                is_relevant, confidence = self.relevance_checker.check_relevance_from_results(search_results)
                if not is_relevant:
    ...
    def _extract_text_around_table(
        ...
        if page_number > 0 and page_number <= len(pages):
            ...
            if paragraphs:
                context_before = "\n\n".join(paragraphs[-2:])[-context_window:] if len(paragraphs) >= 2 else ""
                context_after = "\n\n".join(paragraphs[:2])[:context_window] if len(paragraphs) >= 2 else ""
            else:
                context_before = ""
                context_after = ""
