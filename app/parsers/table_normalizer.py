...
class TableNormalizer:
    ...
    def normalize_from_pdfplumber(
        ...
        if len(raw_table_data) > 0:
            ...
            if first_row and all(isinstance(cell, str) for cell in first_row):
                ...
                if TableNormalizer._looks_like_header_row(
                    potential_headers,
                    raw_table_data[1:] if len(raw_table_data) > 1 else [],
                ):
                    headers = [
                        str(cell).strip() if cell else "" for cell in potential_headers
                    ]
                    data = raw_table_data[1:]

        normalized_data = [
            [str(cell).strip() if cell else "" for cell in row] for row in data
        ]
        ...
        return Table(data=normalized_data, headers=headers, metadata=metadata)
    ...
    def normalize_from_pymupdf(
        ...
        if isinstance(raw_table_data, dict):
            ...
            if TableNormalizer._looks_like_header_row(
                first_row, raw_table_data[1:] if len(raw_table_data) > 1 else []
            ):
    ...
    def normalize_from_unstructured(
        ...
        try:
            ...
            if hasattr(raw_table_data, "metadata") and hasattr(
                raw_table_data.metadata, "text_as_html"
            ):
            ...
            raise ParserError(
                f"Failed to normalize unstructured table: {str(e)}"
            ) from e

    @staticmethod
    def _looks_like_header_row(
        potential_header_row: List[str], data_rows: List[List[str]]
    ) -> bool:
        if not potential_header_row or not data_rows:
            return False

        header_cells = [
            str(cell).strip().lower() for cell in potential_header_row if cell
        ]
        if not header_cells:
            return False
        ...
        header_word_count = sum(
            1 for word in common_header_words if word in header_text
        )
        ...
        if all(len(str(cell).strip()) < 50 for cell in potential_header_row):
            if all(
                not str(cell).strip().replace(".", "").replace(",", "").isdigit()
